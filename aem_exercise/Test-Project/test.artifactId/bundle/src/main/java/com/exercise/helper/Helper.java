...
public class Helper {
	...
	public CommentResponseVO sortComments(JSONObject response) {
        ...
        Collections.sort(commentresponse.getCommentsList(), (firstComment, secondComment) ->firstComment.getComment().compareTo(secondComment.getComment()));
	...
	public BlogResponseVO getBlogs(JSONObject blogsJson, JSONObject commentsJson) {
		BlogResponseVO blogsresponse = (BlogResponseVO) JsonUtil.convertJsonToJavaObject(blogsJson.toString(), BlogResponseVO.class);
		CommentResponseVO commentresponse = (CommentResponseVO) JsonUtil.convertJsonToJavaObject(commentsJson.toString(),
			                CommentResponseVO.class);
		...
		if(null != blogs && !blogs.isEmpty()){
			blogs.forEach(blog->{
				...
				//for each comment id in the blog fetch comment details and populate comment details
		...
		Collections.sort(blogsresponse.getBlogsList(), (firstBlog, secondBlog) ->firstBlog.getBlogName().compareTo(secondBlog.getBlogName()));
